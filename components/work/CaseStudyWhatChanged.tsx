...
export function CaseStudyWhatChanged({ headingId, sectionHeading, themes, items }: Props) {
  const cells = themes.map((title, i) => ({
    title,
    ...items[i],
          ...
          {cells.map(({ title, body, contrast, Icon }) => (
            <article key={title} className="group min-w-0">
              <div className="flex gap-5">
                <Icon
                  className="mt-1 h-[18px] w-[18px] shrink-0 text-primary/[0.28] transition-colors duration-300 group-hover:text-bronze/55"
                  strokeWidth={1.15}
                  aria-hidden="true"
                />
                <div className="min-w-0">
                  ...
                  <p className="mt-4 text-[15px] leading-[1.68] text-primary/[0.72]">{body}</p>
                  {contrast ? (
                    <p className="mt-5 max-w-[52ch] text-[11px] font-medium uppercase tracking-[0.16em] text-primary/38">
